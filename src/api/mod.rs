...
const LIST_PLAYLISTS_URL: &str =
    "https://youtube.googleapis.com/youtube/v3/playlists";
const LIST_PLAYLIST_ITEMS_URL: &str =
...
impl YoutubeApi {
    pub async fn get_video_info(id: &str)
                                -> Result<VideoMetadata, failure::Error> {
        let url =
        ...
        let metadata: VideoMetadata =
            serde_json::from_str(&response.player_response)?;

        Ok(metadata)
    }

    pub fn new<S: Into<String>>(api_key: S) -> Self {
        YoutubeApi { api_key: api_key.into(),
                     oauth: None,
                     client: Client::new() }
    }
    ...
    pub async fn search(&self,
                        ...
                        -> Result<SearchResponse, failure::Error> {
        let request = search_request.build(&self.api_key);
        let response =
            self.client.get(SEARCH_URL).query(&request).send().await?;
    ...
    pub async fn list_playlists(
        ...
        -> Result<ListPlaylistsResponse, failure::Error> {
    ...
    pub async fn list_playlist_items(
        ...
        -> Result<ListPlaylistItemsResponse, failure::Error> {
        let request = request.build();
    ...
    async fn retry_request(&self,
        ...
        if let Some(StatusCode::UNAUTHORIZED) = err.status() {
            oauth.token.refresh(&oauth.client).await?;
            let res = req.bearer_auth(oauth.token.get_auth_header().await?)
                         .send()
                         .await?
                         .error_for_status()?;
            Ok(res)
    ...
    async fn handle_error<TResponse>(response: reqwest::Response)
                                     -> Result<TResponse, Error>
...
#[cfg(test)]
mod test {
    use crate::{models::SearchRequestBuilder, YoutubeApi};
    ...
    async fn search_should_work() {
        let api = create_api();
        let request =
            SearchRequestBuilder { query: Some(String::from("Don't stay \
                                                             in school")),
