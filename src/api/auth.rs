...
use crate::{
    api::YoutubeOAuth,
    auth::{get_oauth_url, perform_oauth, request_token},
    token::AuthToken,
    YoutubeApi
};
...
impl YoutubeApi {
    pub fn new_with_oauth<S: Into<String>>(api_key: S,
                                           client_id: String,
        ...
        let oauth = YoutubeOAuth { client_id,
                                   client_secret,
                                   client: oauth_client,
                                   token: AuthToken::new() };

        Ok(YoutubeApi { api_key: api_key.into(),
                        oauth: Some(oauth),
                        client: Client::new() })
    }
        ...
        where H: Fn(String) -> String
    {
        let oauth =
            self.oauth
                .as_ref()
                .ok_or_else(|| format_err!("OAuth client not configured"))?;
